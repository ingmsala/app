...
class Matricula extends \yii\db\ActiveRecord
{
    ...
    {
        $espaciocurricular = Comision::find()
        ->where(['id' => $this->comision])
        ->one()->espaciocurricular;

        $comisionotra = Espaciocurricular::find()
                            ->joinWith(['comisions','comisions.matriculas'])
                            ->where(['<>', 'comision.id', $this->comision])
                            ->andWhere(['matricula.alumno' => $this->alumno])
                            ->andWhere(['espaciocurricular.id' => $espaciocurricular])->count();
        if ($comisionotra>0)
            $this->addError($attribute, 'El alumno seleccionado ya está inscripto en otra comisión de este Espacio curricular');
    }
